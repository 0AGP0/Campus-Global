...
export interface DitheringShaderProps {
  ...
  pxSize?: number;
  speed?: number;
  /** 0 veya bırak: sınırsız. Örn. 20–24: CPU ciddi düşer, hareket hâlâ akıcı. */
  maxFps?: number;
  /** 0–1; 1 dış alan, &lt;1 (örn. 0.6) aynı görüntü alanına daha az piksel çizilir (GPU+CPU) */
  renderScale?: number;
  /** Görünmeyen sekmede animasyonu durdur */
...
export function DitheringShader({
  ...
  const bufW = Math.max(2, Math.floor(width * renderScale));
  const bufH = Math.max(2, Math.floor(height * renderScale));
  ...
  useEffect(() => {
    ...
    const minFrameMs = maxFps != null && maxFps > 0 && maxFps < 120 ? 1000 / maxFps : 0;
    let lastFrameTime = 0;
    let alive = true;

    const shouldRunFrame = () =>
      speed !== 0 && alive && (!pauseWhenTabHidden || !document.hidden) && (!pauseWhenOffscreen || inViewRef.current);
    ...
    const tick = (t: number) => {
      ...
      if (minFrameMs > 0 && lastFrameTime > 0 && t - lastFrameTime < minFrameMs - 0.2) {
    ...
    const startLoop = () => {
      if (!shouldRunFrame() || animationRef.current !== undefined) return;
      lastFrameTime = 0;
      animationRef.current = requestAnimationFrame(tick);
    };
